...
Rookhold v0.7 retains three explicit operating postures: per-job gVisor OCI on a dedicated Linux x86_64 VM (the guarded production default), the weaker Linux x86_64 namespace fallback, and an unisolated same-trust development subprocess. No mode silently satisfies a stronger requested isolation class.
