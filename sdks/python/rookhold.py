...
class ExecutionRequirements(TypedDict, total=False):
    """Atomic admission requirements enforced by a supporting Rookhold server."""
...
class _RequiredRookholdEvent(TypedDict):
    seq: int
    ts_ms: int
    kind: str
    data: Dict[str, Any]
...
class RookholdEvent(_RequiredRookholdEvent, total=False):
...
class HashedRookholdEvent(_RequiredRookholdEvent):
...
class RookholdError(RuntimeError):
...
class Rookhold:
    ...
    def _request_with_metadata(
        ...
        *,
        ...
        headers = {
            ...
            "User-Agent": f"rookhold-python/{__version__}",
        }
        if data is not None:
        ...
        try:
            ...
            raise RookholdError(str(reason), code=code, retryable=True) from exc
    ...
    def submit_result(
        ...
        **limit_overrides: Union[int, bool],
        ...
        supplied or generated key and assumes the target Rookhold server implements
    ...
    def attestation_public_key(self) -> AttestationPublicKey:
        """Return key material advertised by this authenticated Rookhold server.
    ...
    def list(
        ...
        *,
        ...
        if not isinstance(raw, dict):
            raise RookholdError("invalid job list envelope", code="invalid_response")
    ...
    def _event_page_with_timeout(
        ...
        if isinstance(raw, list):
            events = cast(List[RookholdEvent], raw)
        ...
        if not isinstance(raw, dict):
            raise RookholdError(
                "invalid event replay envelope", code="invalid_response"
            )
        envelope = cast(Dict[str, Any], raw)
        ...
        return {
            "events": cast(List[RookholdEvent], envelope["events"]),
        ...
        *,
        ...
        while True:
            ...
            if cursor is not None and next_cursor <= cursor:
                raise RookholdError(
                    "event cursor did not advance", code="invalid_response"
                )
            cursor = next_cursor
    ...
    @staticmethod
    def _terminal_event(event: RookholdEvent) -> bool:
    ...
    def _to_websocket_url(self, url: str) -> str:
        ...
        if scheme not in ("ws", "wss"):
            raise RookholdError("invalid stream URL", code="invalid_response")
    ...
    def _stream_url(self, job_id: str, after: int, allow_legacy_query_key: bool) -> str:
        ...
        try:
            ...
            if exc.status not in (404, 405):
                raise
            # A structured Rookhold error such as `job_not_found` is not evidence
        ...
        if not isinstance(ticket_value, dict):
            raise RookholdError(
                "invalid stream ticket response", code="invalid_response"
            )
        ...
        if prefer_websocket:
            ...
            try:
                ...
                while True:
                    ...
                    event = cast(RookholdEvent, json.loads(raw))
                    seq = int(event.get("seq", -1))
                    if seq <= cursor:
                        continue
                    cursor = seq
