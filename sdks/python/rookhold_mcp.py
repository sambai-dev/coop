"""Dependency-free stdio MCP adapter for the Rookhold execution gateway.

The adapter deliberately keeps the Rookhold URL and bearer key outside model-visible
tool arguments. It implements the stable newline-delimited stdio transport so
the same installed ``rookhold-mcp`` command works with Hermes, OpenClaw, and other
...
TOOLS: List[JsonObject] = [
    {
        ...
        "title": "Run code with Rookhold",
        "description": (
            "Run one short Python, Node.js, or Bash job under Rookhold policy and return "
        ...
        "inputSchema": _object_schema(
            {
                ...
                "wait_seconds": {
                    ...
                    "description": (
                        ...
                        "running if this wait expires; use rookhold_job_result afterward."
                    ),
                },
            },
            ("language", "code"),
    ...
    },
    {
        ...
        "title": "Get a Rookhold job result",
        "description": (
            "Wait for a previously submitted Rookhold job and return its bounded terminal "
    ...
    {
        "name": "rookhold_job_events",
        "title": "Inspect Rookhold job evidence",
        "description": (
            "Read a cursor-bounded page of persisted lifecycle, output, violation, and "
            "terminal evidence events for a Rookhold job."
        ),
        "inputSchema": _object_schema(
            {
                "job_id": {"type": "string", "minLength": 1},
    ...
    },
    {
        ...
        "title": "Cancel a Rookhold job",
        "description": "Cancel one queued or running Rookhold job owned by this tenant.",
        "inputSchema": _object_schema(
    ...
    def __init__(
        ...
        *,
        ...
        if not allowed or not allowed.issubset(LANGUAGES):
            raise ValueError(
                "ROOKHOLD_MCP_ALLOWED_LANGUAGES must be a non-empty comma-separated "
                "subset of python,node,bash"
            )
        if not 1 <= max_wait_seconds <= 300:
            raise ValueError("ROOKHOLD_MCP_MAX_WAIT_SECONDS must be between 1 and 300")
        if not 1 <= max_code_bytes <= 1024 * 1024:
            raise ValueError(
                "ROOKHOLD_MCP_MAX_CODE_BYTES must be between 1 and 1048576"
            )
        if not 1 <= task_ttl_ms <= DEFAULT_TASK_TTL_MS:
            raise ValueError("ROOKHOLD_MCP_TASK_TTL_MS must be between 1 and 3600000")
        isolation_classes = {
            "none",
            "linux-shared-kernel",
        ...
        if minimum_isolation is not None and minimum_isolation not in isolation_classes:
            raise ValueError(
                "ROOKHOLD_MCP_MINIMUM_ISOLATION must be an exact Rookhold isolation class"
            )
        ...
        if require_isolation:
            if resolved_isolation == "none":
                ...
                raise ValueError(
                    "ROOKHOLD_MCP_REQUIRE_ISOLATION is incompatible with the separate "
    ...
    def from_env(cls) -> "McpConfig":
        languages = [
            ...
            for value in _env_value(
                "ROOKHOLD_MCP_ALLOWED_LANGUAGES", "python,node,bash"
            ).split(",")
            if value.strip()
        ]
        return cls(
            base_url=_env_value("ROOKHOLD_BASE_URL", DEFAULT_BASE_URL),
            api_key=_env_value("ROOKHOLD_API_KEY", ""),
            allowed_languages=languages,
            max_wait_seconds=_env_int(
                "ROOKHOLD_MCP_MAX_WAIT_SECONDS", DEFAULT_MAX_WAIT_SECONDS
            ),
            max_code_bytes=_env_int(
                "ROOKHOLD_MCP_MAX_CODE_BYTES", DEFAULT_MAX_CODE_BYTES
            ),
            require_isolation=_env_bool("ROOKHOLD_MCP_REQUIRE_ISOLATION", False),
            minimum_isolation=cast(
                Optional[IsolationClass],
                _env_value("ROOKHOLD_MCP_MINIMUM_ISOLATION", None),
            ),
            enable_tasks=_env_bool("ROOKHOLD_MCP_ENABLE_TASKS", False),
            task_ttl_ms=_env_int("ROOKHOLD_MCP_TASK_TTL_MS", DEFAULT_TASK_TTL_MS),
        )
...
@overload
def _env_value(name: str, default: None) -> Optional[str]: ...


def _env_value(name: str, default: Optional[str]) -> Optional[str]:
    legacy_name = (
        "COOP_" + name[len("ROOKHOLD_") :] if name.startswith("ROOKHOLD_") else None
    )
    primary = os.environ.get(name)
    legacy = os.environ.get(legacy_name) if legacy_name is not None else None
    if primary not in {None, ""} and legacy not in {None, ""} and primary != legacy:
        raise ValueError(
            f"{name} conflicts with legacy compatibility variable {legacy_name}"
        )
    if primary not in {None, ""}:
        return primary
    if legacy is not None:
        return legacy
    return primary if primary is not None else default


def _env_int(name: str, default: int) -> int:
    raw = _env_value(name, None)
    if raw is None:
        return default
    try:
        return int(raw)
...
def _submission_fingerprint(
    ...
    *,
    ...
    return hmac.new(
        secret,
        # Preserve the v0.5 domain separator so ambiguous submissions reconcile
        # to the same idempotency key across the product rename.
...
class RookholdMcpServer:
    """Dual-era MCP server whose tool handlers call one Rookhold tenant."""

    def __init__(self, config: McpConfig, client: Optional[Rookhold] = None) -> None:
        self.config = config
        self.client = client or Rookhold(config.base_url, config.api_key)
    ...
    def _server_info() -> JsonObject:
        return {"name": "rookhold-mcp", "version": __version__}

    @staticmethod
    def _instructions() -> str:
        return (
            "Use rookhold_run_code for short stateless jobs. Do not use it for "
            "persistent workspaces or interactive shells. The Rookhold URL, key, "
    ...
    def _listed_tools(self, *, modern: bool = False) -> List[JsonValue]:
        ...
        tools[0]["description"] = (
            f"Run one short job on the {posture} Rookhold backend and return its bounded "
    ...
    def _call_tool(
        ...
        *,
        ...
        try:
            if name in {"rookhold_run_code", "coop_run_code"}:
    ...
    def _validate_limits(
        ...
        if network is True:
            raise ValueError("allow_network=true is not supported by Rookhold policy")
    ...
    def _run_code(
        ...
        *,
        ...
        if language not in capabilities["languages"]:
            raise ValueError(
                f"runtime is unavailable on the Rookhold server: {language}"
            )
        ...
        except RookholdError:
            self._submission_reconciler.finish(
                claim,
                accepted=False,
                # submit_result() exposes only the final error. Any RookholdError can
        ...
        if not isinstance(submission_value, dict):
            ...
            raise RuntimeError("Rookhold returned invalid submission metadata")
        ...
        if not isinstance(job_id_value, str) or not job_id_value:
            self._submission_reconciler.finish(claim, accepted=False, unresolved=True)
            raise RuntimeError("Rookhold returned an invalid submitted job ID")
        job_id = job_id_value
        ...
        if create_task:
            ...
            return {
                ...
                "taskId": job_id,
                "status": "working",
                "statusMessage": "The Rookhold job was durably accepted.",
        ...
        try:
            ...
            failure: JsonObject = {
                ...
                "message": (
                    ...
                    "Call rookhold_job_result with this job_id or cancel it."
                ),
                "error_code": (
                    exc.code if isinstance(exc, RookholdError) else type(exc).__name__
                ),
                "retryable": (
                    exc.retryable
                    if isinstance(exc, RookholdError)
    ...
    def _task_get(self, params: JsonObject) -> JsonObject:
        ...
        if status in {"queued", "running"}:
            task["status"] = "working"
            task["statusMessage"] = f"Rookhold job is {status}."
            return task
        if status == "cancelled":
            task["status"] = "cancelled"
            task["statusMessage"] = (
                "Rookhold job cancellation reached a terminal state."
            )
            return task
        if status not in {
            "succeeded",
            ...
            raise RuntimeError(f"unknown Rookhold job status: {status}")
        ...
        task["statusMessage"] = f"Rookhold job completed with status {status}."
    ...
    def _task_update(self, params: JsonObject) -> JsonObject:
        ...
        # Rookhold execution jobs never request client input. Resolving the job is
...
def serve(
    ...
    workers = concurrent.futures.ThreadPoolExecutor(
        ...
        thread_name_prefix="rookhold-mcp-request",
    )
    cancellations = concurrent.futures.ThreadPoolExecutor(
        max_workers=2,
        thread_name_prefix="rookhold-mcp-cancel",
    )
...
def main() -> None:
    try:
        ...
        print(f"rookhold-mcp: {exc}", file=sys.stderr)
